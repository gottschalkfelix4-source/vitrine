...
FELDER: list[Feld] = [
    ...
    Feld("hwaccel", "Kaltspeicher", "Hardware-Encoder", "Vielfach schneller, dafür meist etwas weniger Qualität je Byte. Achtung: Intel 11.–13. Generation kann AV1 nur DEkodieren; für AV1 in Hardware braucht es Arc, Meteor/Lunar/Arrow Lake, RTX 40/50 oder RDNA3+. Für qsv/vaapi muss /dev/dri in den Container gereicht sein – ob das alles stimmt, sagt die Prüfung oben.", "auswahl", auswahl=[h.value for h in HardwareAccel], pruefen=_codec_pruefen),
    Feld("hwaccel_device", "Kaltspeicher", "Grafikkarte (Gerät)", "Nur für vaapi. Steckt neben der Arc noch eine iGPU im Rechner, ist die Arc womöglich /dev/dri/renderD129 – welcher Knoten der richtige ist, sagt die Prüfung oben.", "text"),
    ...
    # -------------------------------------------------------------------- VPN
    Feld("vpn_aktiv", "VPN", "Tunnel benutzen", "Wirkt sofort. Aus: Alles läuft über die eigene Leitung – wie bisher. An: Jeder Download nimmt reihum einen der eingerichteten Tunnel. Der Sinn ist Tempo, nicht Verschleierung – YouTube zählt je IP-Adresse, vier Tunnel sind vier Budgets. Ohne eingerichteten Tunnel bewirkt der Schalter nichts.", "bool"),
    Feld("vpn_nur_tunnel", "VPN", "Nur über Tunnel laden", "An lassen. Aus bedeutet: Fallen alle Tunnel aus, lädt das Archiv stillschweigend wieder über die Hausleitung – also genau über die Adresse, die man herausnehmen wollte, und ohne dass es auffällt.", "bool"),
