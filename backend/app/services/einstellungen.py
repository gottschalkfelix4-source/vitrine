...
FELDER: list[Feld] = [
    ...
    Feld("hwaccel", "Kaltspeicher", "Hardware-Encoder", "Gilt für die Archivierung und neue Live-Wiedergaben. none verwendet die CPU. Den tatsächlich verwendeten Encoder und einen möglichen CPU-Rückfall zeigt das Stream-Dashboard. Für qsv/vaapi muss /dev/dri in den Container gereicht sein. Die Prüfung oben testet den Archivcodec; H.264 für Live-Wiedergaben unterstützen auch viele ältere GPUs ohne AV1-Encoder.", "auswahl", auswahl=[h.value for h in HardwareAccel], pruefen=_codec_pruefen),
    Feld("hwaccel_device", "Kaltspeicher", "Grafikkarte (Gerät)", "Für vaapi sowie qsv bei Live-Wiedergaben unter Linux. Steckt neben der Arc noch eine iGPU im Rechner, ist die Arc womöglich /dev/dri/renderD129. Die Hardware-Prüfung oben zeigt die verfügbaren Geräte.", "text"),
